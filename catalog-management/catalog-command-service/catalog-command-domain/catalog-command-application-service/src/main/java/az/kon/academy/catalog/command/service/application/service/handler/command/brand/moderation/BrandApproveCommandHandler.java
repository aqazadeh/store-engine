...
import static az.kon.academy.catalog.command.service.application.service.constant.SecurityPermissions.Brand.BRAND_MODERATION_APPROVE;
...
@CommandHandler(roles = ROLE_DOMAIN_MODERATOR, permissions = BRAND_MODERATION_APPROVE)
public class BrandApproveCommandHandler implements AbstractCommandHandler<BrandApproveCommand, Void> {
    ...
    public Void handle(BrandApproveCommand command) {
        var brand = this.brandModerationDomainService.approve(domainContext, command);
