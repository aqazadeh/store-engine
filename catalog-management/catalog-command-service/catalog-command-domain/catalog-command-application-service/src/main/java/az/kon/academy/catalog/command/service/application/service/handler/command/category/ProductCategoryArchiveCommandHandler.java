...
import static az.kon.academy.catalog.command.service.application.service.constant.SecurityPermissions.ProductCategory.PRODUCT_CATEGORY_MODERATION_ARCHIVE;
...
@CommandHandler(roles = ROLE_DOMAIN_MODERATOR, permissions = PRODUCT_CATEGORY_MODERATION_ARCHIVE)
public class ProductCategoryArchiveCommandHandler implements AbstractCommandHandler<ProductCategoryArchiveCommand, Void> {
    ...
    public Void handle(ProductCategoryArchiveCommand command) {
        var productCategory = this.productCategoryModerationDomainService.archive(domainContext, command);
