...
import static az.kon.academy.catalog.command.service.application.service.constant.SecurityPermissions.ProductCategory.PRODUCT_CATEGORY_MODERATION_ACTIVATE;
...
@CommandHandler(roles = ROLE_DOMAIN_MODERATOR, permissions = PRODUCT_CATEGORY_MODERATION_ACTIVATE)
public class ProductCategoryActivateCommandHandler implements AbstractCommandHandler<ProductCategoryActivateCommand, Void> {
    ...
    public Void handle(ProductCategoryActivateCommand command) {
        var productCategory = this.productCategoryModerationDomainService.activate(domainContext, command);
